...
import { LayoutDashboard, FileEdit, Users, MessageCircle, Settings, LogOut, PlusCircle } from "lucide-react";
  ...
  return (
    <div className="flex bg-slate-900 text-white w-64 min-h-screen flex-col">
      <div className="p-6 border-b border-slate-800">
        <h1 className="text-xl font-bold bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">PRizm Client</h1>
      ...
      <nav className="flex-1 p-4 space-y-2">
        ...
        <Link to="/client/campaigns/new"
          ...
          <PlusCircle className="w-5 h-5" /><span>新規案件作成</span>
        </Link>
      </nav>
      <div className="p-4 border-t border-slate-800">
        <Link to="/client/settings"
          className={cn("flex items-center space-x-3 px-4 py-3 transition-colors text-sm rounded-lg",
            location.pathname === "/client/settings" ? "bg-blue-600 text-white" : "text-slate-400 hover:text-white")}>
