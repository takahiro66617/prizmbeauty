...
export interface ExternalApplication {
  ...
  applied_at: string;
  updated_at: string;
  campaigns?: { id: string; title: string; image_url: string | null; budget_min: number | null; budget_max: number | null; deadline: string | null; category: string | null; description: string | null; deliverables: string | null; platform: string | null; requirements: string | null; companies?: { id: string; name: string } | null } | null;
...
export function useExternalApplications(filters?: { companyId?: string; influencerId?: string; campaignId?: string }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        .from("applications")
        .select("*, campaigns(id, title, image_url, budget_min, budget_max, deadline, category, description, deliverables, platform, requirements, companies(id, name)), influencer_profiles(id, name, username, image_url, instagram_followers, tiktok_followers, youtube_followers, category, bio, status, user_id, instagram_url, tiktok_url, youtube_url, twitter_url)")
