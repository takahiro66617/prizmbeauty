...
import { Heart, Clock, Users, CheckCircle, Instagram, Youtube, ArrowLeft, FileText } from "lucide-react";
...
export default function CampaignDetail() {
  ...
  // Check if navigated from mypage
  const isFromMyPage = document.referrer.includes("/mypage") || window.location.pathname.includes("/mypage");
              ...
              <div className="flex items-center gap-4 py-4 border-t border-b border-border">
                <div className="w-12 h-12 bg-muted rounded-full flex items-center justify-center font-bold text-xl text-muted-foreground overflow-hidden">
                  {campaign.companies?.logo_url ? (
                    <img src={campaign.companies.logo_url} alt="" className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {platforms.map((p, i) => (
                        <span key={i} className="px-3 py-1 bg-muted rounded-full text-sm font-medium">{p}</span>
                      ))}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex justify-between items-center text-sm pt-4 border-t border-border">
                    ...
                    <span className={`font-bold ${daysRemaining !== null && daysRemaining <= 7 ? "text-destructive" : ""}`}>
                      ...
                      {daysRemaining !== null && daysRemaining >= 0 && (
                        <span className="text-xs ml-1">(あと{daysRemaining}日)</span>
                      )}
                    </span>
                  </div>
                )}
