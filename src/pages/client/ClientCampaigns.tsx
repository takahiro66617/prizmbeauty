...
import { useExternalCampaigns, useDeleteCampaign } from "@/hooks/useExternalCampaigns";
import { useExternalApplications } from "@/hooks/useExternalApplications";
import { CATEGORIES, CAMPAIGN_STATUSES } from "@/lib/constants";
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">自社の案件一覧を管理します。行クリックで詳細・編集ページへ。</p>
        </div>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-gray-200">
            {isLoading ? (
              ...
              return (
                <tr key={campaign.id} className={`hover:bg-gray-50 cursor-pointer ${overdue ? "bg-red-50" : ""}`}
                  ...
                  <td className="px-6 py-4" onClick={e => e.stopPropagation()}>
                    <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50"
                      onClick={() => {
                        if (window.confirm(`「${campaign.title}」を削除しますか？`)) {
